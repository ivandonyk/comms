...
const getMentionedPostsInChannel = async (channelId, currentUserId) => (await db
...
const getOtherUsers = async (currentUserId) => await db.collection("users").where("uid", "!=", currentUserId).get();
...
const addToInbox = (user, post) => {
    db.collection(`users/${user.uid}/inbox`)
        ...
        .set(post)
        .then((doc) => {
        functions.logger.info("Post added to inbox successfully!, userId => ", user.uid, "postId => ", post.id);
    });
};
...
const editInboxPost = (user, post, key, value) => {
    db.collection(`users/${user.uid}/inbox`)
        ...
        .then((doc) => {
        functions.logger.info("Inbox post updated successfully!, userId => ", user.uid, "postId => ", post.id, ",", key, "changed to =>", value);
    });
};
...
const checkNotificationPreference = async (user, post) => {
    ...
    if (channelPreference === "involved") {
        ...
        const channelPosts = await getMentionedPostsInChannel(post.channelId, user.uid);
...
exports.onPostCreated = functions.firestore
    ...
    return users.forEach((doc) => checkNotificationPreference(doc.data(), post));
});
